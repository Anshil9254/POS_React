...
const PromoCodeEdit = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            try {
                // Fetch form options and promo code concurrently
                const [optionsRes, promoRes] = await Promise.all([
                    api.get('/promocodes/form-data'),
                ...
                setFormData({
                    ...
                    start_date: formatDateStr(promo.start_date),
                    end_date: formatDateStr(promo.end_date),
    ...
    return (
        <div className="container mx-auto px-4 max-w-[1400px]">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6">
                <h1 className="text-[1.75rem] font-medium text-[#5a5c69] mb-0 font-montserrat">Edit Promo Code</h1>
                            ...
                            <div className="bg-white rounded shadow-[0_0.15rem_1.75rem_0_rgba(58,59,69,0.15)] border-0 overflow-hidden">
                                ...
                                <div className="p-5">
                                    <div className="mb-4">
                                        ...
                                        <input type="text" name="code" value={formData.code} onChange={handleChange} required className="w-full px-3 py-2 bg-white border border-[#d1d3e2] rounded text-[#6e707e] focus:outline-none focus:ring-1 focus:ring-[#4e73df] focus:border-[#4e73df] transition-all uppercase" />
                                    </div>
                                    <div className="flex flex-col md:flex-row gap-4">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <input type="number" name="value" value={formData.value} onChange={handleChange} required step="0.01" className="w-full px-3 py-2 bg-white border border-[#d1d3e2] rounded text-[#6e707e] focus:outline-none focus:ring-1 focus:ring-[#4e73df] focus:border-[#4e73df] transition-all" />
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-white rounded shadow-[0_0.15rem_1.75rem_0_rgba(58,59,69,0.15)] border-0 overflow-hidden">
                                ...
                                <div className="p-5">
                                    <div className="border border-[#e3e6f0] p-3 rounded mb-4">
                                        <div className="flex items-center">
                                            <div className="relative inline-block w-12 mr-2 align-middle select-none transition duration-200 ease-in">
                                                ...
                                                />
                                                <label htmlFor="togglePromoEdit" className={`toggle-label block overflow-hidden h-6 rounded-full bg-[#eaecf4] cursor-pointer ${formData.is_active ? 'bg-[#1cc88a]' : ''}`}></label>
                                            </div>
                                            <label htmlFor="togglePromoEdit" className={`text-base font-bold cursor-pointer transition-colors ${formData.is_active ? 'text-[#1cc88a]' : 'text-[#858796]'}`}>
                                                Active Promo Code
                                            </label>
                                        </div>
                                    </div>
                                    ...
                                    <button type="submit" disabled={isSubmitting} className="w-full flex items-center justify-center px-4 py-3 text-base bg-[#4e73df] hover:bg-[#2e59d9] text-white rounded font-normal transition-colors shadow-sm cursor-pointer disabled:opacity-75 disabled:cursor-not-allowed">
                                        <i className="fas fa-save fa-sm text-white/50 mr-2"></i> {isSubmitting ? 'Updating...' : 'Update Promo Code'}
                                    </button>
                                </div>
                            </div>
