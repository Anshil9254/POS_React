...
const LowStockReport = () => {
    ...
    const fetchReport = async () => {
        setIsLoading(true);
        try {
            const response = await api.get('/reports/lowstock', {
    ...
    const stockItems = reportData.map(item => {
    ...
    const summary = {
        ...
        totalStockValue: stockItems.reduce((acc, item) => acc + parseFloat(item.stockValue), 0).toFixed(2),
        ...
        <>
            ...
            <div className="container mx-auto px-4 py-4 max-w-[1400px] print:hidden">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                    <h2 className="text-[2rem] font-bold text-[#212529] m-0 font-montserrat">Low Stock Report</h2>
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                            <div className="bg-white rounded shadow-sm border border-[#e3e6f0]">
                                <div className="p-4">
                                    <h6 className="text-[#6c757d] mb-2 text-[1rem]">Total Problem Items</h6>
                                    <h3 className="text-[1.75rem] font-medium text-[#212529] m-0">{summary.totalProducts}</h3>
                                </div>
                            </div>
                            <div className="bg-white rounded shadow-sm border border-[#e3e6f0]">
                                <div className="p-4">
                                    <h6 className="text-[#6c757d] mb-2 text-[1rem]">Remaining Stock</h6>
                            ...
                            <div className="bg-[#ffc107] text-white rounded shadow-sm">
                                <div className="p-4">
                                    <h6 className="text-white/80 mb-2 text-[1rem]">Low/Out of Stock</h6>
                                    <h3 className="text-[1.75rem] font-medium text-white m-0">{summary.lowStockCount} / {summary.outOfStockCount}</h3>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded shadow-sm border border-[#e3e6f0]">
                            <div className="bg-[#f8f9fa] border-b border-[#e3e6f0] px-4 py-3 border-t">
                                <h5 className="m-0 font-medium text-[#212529] text-[1.25rem]">Low Stock Items Detail</h5>
                            </div>
                            <div className="p-0 overflow-x-auto">
                                {stockItems.length > 0 ? (
                                        ...
                                        <tbody>
                                            {stockItems.map((item) => (
                                                <tr key={item.id} className="border-b border-[#dee2e6] hover:bg-[#f8f9fa] transition-colors">
                                                    ...
                                                    <td className="p-3 text-center text-[#212529] font-bold text-red-500">{item.quantity}</td>
                                                    ...
                                                    <td className="p-3">
                                                        <span className={`inline-block px-2 py-1 text-xs font-semibold rounded ${item.status === 'Low Stock' ? 'bg-[#ffc107] text-black' : 'bg-[#dc3545] text-white'
                                                            }`}>
                                                            {item.status}
                                                        </span>
                                                    </td>
                                    ...
                                    <div className="py-12 text-center">
                                        ...
                                        <p className="text-[#6c757d]">All stock levels are optimal</p>
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            <div id="print-area" style={{ position: 'absolute', width: 0, height: 0, overflow: 'hidden' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', borderBottom: '2px solid #000', paddingBottom: '8px', marginBottom: '12px' }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                        <div style={{ fontSize: '13pt', fontWeight: '700', color: '#000' }}>Low Stock Report</div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '10px', marginBottom: '14px' }}>
                    {[
                        { label: 'Problem Items', value: summary.totalProducts },
                        { label: 'Remaining Stock', value: `${summary.totalStock} units` },
                        ...
                        <div key={label} style={{ border: '1px solid #000', padding: '8px 12px' }}>
                    ...
                    <tbody>
                        {stockItems.length > 0 ? stockItems.map((item) => (
                            <tr key={item.id}>
                                ...
                                <td style={{ padding: '4px 6px', color: '#000', borderBottom: '1px solid #ccc', borderRight: '1px solid #ccc', textAlign: 'center', fontWeight: '700' }}>{item.quantity}</td>
                            ...
                            <tr>
                                <td colSpan="8" style={{ padding: '20px', textAlign: 'center', color: '#000' }}>All stock levels are optimal.</td>
                            </tr>
                        )}
                    </tbody>
                </table>
